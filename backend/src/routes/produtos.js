...
// GET /api/produtos  catalogo publico
router.get("/", async (req, res) => {
  try {
    ...
    const produtos = await query.select("id","nome","descricao","emoji","foto_url","categoria","unidade","preco","estoque_max","badge","permite_delivery");
...
router.get("/admin", authStaff, async (req, res) => {
  try {
    const { categoria, busca } = req.query;
    let query = db("produtos").where({ padaria_id: req.staff.padaria_id }).orderBy("ordem","asc").orderBy("nome","asc");
    if (categoria && categoria !== "todos") query = query.where({ categoria });
    if (busca) query = query.whereILike("nome", "%" + busca + "%");
...
router.post("/", authStaff, authDono, async (req, res) => {
  try {
    const { nome, descricao, emoji, foto_url, categoria, unidade, preco, estoque_max, permite_delivery, alergenos, nota_producao, badge, ordem } = req.body;
    ...
    const [produto] = await db("produtos").insert({
      padaria_id: req.staff.padaria_id, nome: nome.trim(),
      descricao: descricao?.trim() || null, emoji: emoji || "", foto_url: foto_url || null,
      categoria: categoria || "classicos", unidade: unidade || "unidade",
      preco: Number(preco), estoque_max: estoque_max || 100,
      permite_delivery: permite_delivery !== false,
      alergenos: alergenos || null, nota_producao: nota_producao || null,
      badge: badge || null, ordem: ordem || 0,
    }).returning("*");
...
router.put("/:id", authStaff, authDono, async (req, res) => {
  try {
    ...
    const campos = ["nome","descricao","emoji","foto_url","categoria","unidade","preco","estoque_max","ativo","disponivel_hoje","permite_delivery","alergenos","nota_producao","badge","ordem"];
...
router.patch("/:id/disponibilidade", authStaff, async (req, res) => {
  try {
    await db("produtos").where({ id: req.params.id, padaria_id: req.staff.padaria_id })
...
router.delete("/:id", authStaff, authDono, async (req, res) => {
  try {
    const atualizado = await db("produtos").where({ id: req.params.id, padaria_id: req.staff.padaria_id })
